...
export function EventForm({ poolType, event, onSave, onCancel }: Props) {
  ...
  useEffect(() => {
    fetch(`/api/event-types?poolType=${poolType}&trainingOnly=false`)
      .then((r) => r.json())
      .then(setEventTypes)
  }, [poolType])
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-4">
      ...
      <div>
        ...
        <select
          value={eventTypeId}
          onChange={(e) => { setEventTypeId(e.target.value); setRelayLeg(''); setRelayStroke('') }}
          required
      ...
      <div className="grid grid-cols-2 gap-4">
        ...
        <div className={isRelay ? '' : 'col-span-2'}>
          <label className="block text-sm font-medium text-gray-700 mb-1">
            ...
            <span className="text-gray-400 font-normal">(mm:ss,hh)</span>
          </label>
          <TimeInput value={timeInput} onChange={setTimeInput} error={timeError} />
        </div>

        {/* Gesamtzeit (nur Staffel) */}
        {isRelay && (
          <div>
            <label className="block text-sm font-medium text-gray-700 mb-1">
              Gesamtzeit Team <span className="text-gray-400 font-normal">(opt.)</span>
            </label>
      ...
      {/* Staffel-Zusatzfelder */}
      {isRelay && (
        <div className="grid grid-cols-2 gap-4">
          <div>
            <label className="block text-sm font-medium text-gray-700 mb-1">Position in Staffel</label>
            <select
              value={relayLeg}
              onChange={(e) => setRelayLeg(e.target.value)}
              className="w-full rounded-md border border-gray-300 px-3 py-2 text-sm
                focus:outline-none focus:ring-2 focus:ring-blue-500"
            >
              <option value="">—</option>
              {[1, 2, 3, 4].map((n) => (
                <option key={n} value={n}>{n}. Schwimmer</option>
              ))}
            </select>
          </div>
          ...
          {isLagenStaffel && (
            <div>
              <label className="block text-sm font-medium text-gray-700 mb-1">Eigene Lage</label>
              <select
                value={relayStroke}
                onChange={(e) => setRelayStroke(e.target.value)}
                className="w-full rounded-md border border-gray-300 px-3 py-2 text-sm
                  focus:outline-none focus:ring-2 focus:ring-blue-500"
              >
                <option value="">—</option>
      ...
      <div className="grid grid-cols-3 gap-4">
        {/* Lauf */}
        <div>
          <label className="block text-sm font-medium text-gray-700 mb-1">Lauf</label>
          <input
            type="text"
            value={heat}
            onChange={(e) => setHeat(e.target.value)}
            list="heat-options"
            placeholder="z.B. Finale"
            className="w-full rounded-md border border-gray-300 px-3 py-2 text-sm
              focus:outline-none focus:ring-2 focus:ring-blue-500"
          />
        ...
        {/* Bahn */}
        <div>
          <label className="block text-sm font-medium text-gray-700 mb-1">Bahn</label>
          <input
            type="number"
            value={lane}
        ...
        {/* Platz */}
        <div>
          <label className="block text-sm font-medium text-gray-700 mb-1">Platz</label>
          <input
            type="number"
            value={place}
