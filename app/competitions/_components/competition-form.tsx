...
export type CompetitionFormData = {
  name: string; location: string; date: string; poolType: PoolType; notes: string
}
...
export function CompetitionForm({ competition, onSave, onCancel }: Props) {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try { await onSave({ name, location, date, poolType, notes }) }
      ...
      <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
        <div className="sm:col-span-2">
          ...
          <input type="text" value={name} onChange={(e) => setName(e.target.value)} required
            placeholder="z.B. Stadtmeisterschaften 2026" className={INPUT} />
        </div>
        <div>
        ...
        <div>
          <label className="block text-sm font-medium text-gray-700 dark:text-slate-300 mb-1">Ort</label>
          <input type="text" value={location} onChange={(e) => setLocation(e.target.value)}
            placeholder="z.B. Hallenbad München" className={INPUT} />
        </div>
        <div className="sm:col-span-2">
            ...
            {(['KURZBAHN', 'LANGBAHN'] as PoolType[]).map((pt) => (
              <label key={pt} className="flex items-center gap-2 cursor-pointer">
                ...
                <span className="text-sm text-gray-700 dark:text-slate-300">
                  {pt === 'KURZBAHN' ? 'Kurzbahn (25m)' : 'Langbahn (50m)'}
                </span>
              </label>
            ))}
          </div>
